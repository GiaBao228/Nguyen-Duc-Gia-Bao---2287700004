...
def scan():
    ...
    if mode in ['scan', 'all']:
        # Lưu ý: async_scan_ports trong hình ảnh có lẽ cần được sửa đổi
        # để trả về kết quả quét (ví dụ: danh sách cổng mở) thay vì chạy trực tiếp.
        # Dựa trên cách code được viết, nó có vẻ chỉ chạy hàm async mà không lưu kết quả rõ ràng.
        # Ở đây chỉ scan mà không lưu kết quả, nên sẽ để trống:
        # result['scan'] = asyncio.run(port_scanner.async_scan_ports(target, ports))
        pass # Nếu không chỉnh sửa port_scanner, kết quả sẽ không được lưu vào 'scan'
    ...
    if mode in ['banner', 'all']:
        result['banner'] = {
            port: banner_grabber.grab_banner(target, port) for port in ports
        }
