...
class CADemoApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Button(btn_frame, text="1. Tạo Root & Intermediate CA",
                  command=self.setup_ca).grid(row=0, column=0, padx=5, pady=5)

        tk.Button(btn_frame, text="2. Phát hành User Cert",
                  command=self.issue_cert).grid(row=0, column=1, padx=5, pady=5)

        tk.Button(btn_frame, text="3. Kiểm tra Chuỗi Cert",
                  command=self.verify_chain).grid(row=1, column=0, padx=5, pady=5)

        tk.Button(btn_frame, text="4. Thu hồi User Cert",
                  command=self.revoke_cert).grid(row=1, column=1, padx=5, pady=5)

        tk.Button(btn_frame, text="5. Kiểm tra Trạng thái OCSP",
                  command=self.ocsp_check).grid(row=2, column=0, columnspan=2, pady=5)
    ...
    def verify_chain(self):
        cert_path = os.path.join("certs", "giabao_cert.pem")
        ...
        self.log(f"Kết quả: Chuỗi chứng chỉ hợp lệ: {valid}")
        messagebox.showinfo("Kết quả", f"Chuỗi hợp lệ: {valid}")

    def revoke_cert(self):
        cert_file   = os.path.join("certs", "giabao_cert.pem")
    ...
    def ocsp_check(self):
        cert_file = os.path.join("certs", "nguyencanhduong_cert.pem")
        if not os.path.exists(cert_file):
            messagebox.showerror("Lỗi", "Chưa có chứng chỉ user để kiểm tra OCSP!")
            return

        self.log("Kiểm tra trạng thái OCSP (dùng CRL)...")
        status = check_revocation_status(cert_file)
        self.log(f"Kết quả OCSP: {status}")
        messagebox.showinfo("Kết quả OCSP", f"Trạng thái: {'Đã thu hồi' if status=='Revoked' else 'Hợp lệ'}")
